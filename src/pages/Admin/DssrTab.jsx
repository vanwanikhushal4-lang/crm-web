...
export default function DssrTab() {
  ...
  const salesManagerRepsList = useMemo(() => {
    return salesManagerOptions.filter((m) => m.role === 'Sales Manager');
  }, [salesManagerOptions]);

  const outsiderRepsList = useMemo(() => {
    return salesManagerOptions.filter((m) => m.role === 'Outsider');
  }, [salesManagerOptions]);
  ...
  const filteredDropdownList = useMemo(() => {
    ...
    return list;
  }, [selectedRoleFilter, salesManagerRepsList, outsiderRepsList, salesManagerOptions, roleSearchQuery]);
  ...
  return (
    <div className="space-y-6 text-slate-100 relative z-20 pb-20 animate-fade">
      ...
      {activeSubTab !== 'dashboard' && (
        <div className="flex flex-col gap-4 bg-[#0b1628]/80 border border-white/10 p-4.5 rounded-2xl shadow-xl animate-fade relative z-40 backdrop-blur-md">
          ...
          <div className="pt-3 border-t border-white/5 flex flex-col gap-3">
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
              ...
              {/* Role Category Toggle Pills with Interactive Dropdown Popover */}
                    ...
                    <div className="max-h-60 overflow-y-auto hide-scrollbar flex flex-col gap-1.5 pr-1">
                      {filteredDropdownList.length === 0 ? (
                        ...
                        filteredDropdownList.map((m) => {
                          const isSelected = selectedUserFilter === m.id || selectedUserFilter === m.name;
                          return (
                            <button
                              key={m.id}
                              onClick={() => {
                                setSelectedUserFilter(m.id);
                                setIsRoleDropdownOpen(false);
                              }}
